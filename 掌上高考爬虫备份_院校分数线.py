...
"""
get_config:根据当前页码和学校ID构建请求参数、请求头和目标URL。
参数:
    page: 当前请求的页码
...
def get_config(page, school_id):
    params = {
        "local_batch_id": 14,
        "local_province_id": "43",  #在该省份的招生情况，41为河南，43为湖南
        "local_type_id": "2073",
        "page": page,
        "school_id": school_id,
        "sg_xuanke":"",
        ...
        "year": 2024,
        "signsafe" : "746698ac890bc8948ecf5c0cf9537dd7",
    }
    headers = {
    ...
    url = "https://api.zjzw.cn/web/api/?e_sort=zslx_rank,min&e_sorttype=desc,desc&local_province_id=43&local_type_id=2073&page=1&school_id=459&size=10&uri=apidata/api/gk/score/province&year=2024&signsafe=f119b82d1d8e6df1d2ba62f9061f231b".format(
        page, school_id)
    return params, headers, url

"""
request_api:发起POST请求，并返回响应的JSON数据。
参数:
    url: 请求的URL
...
"""
get_data:解析每一页返回的专业数据，将其中需要的字段提取出来，并存储到全局字典中。
参数:
    data: 全局数据存储字典
